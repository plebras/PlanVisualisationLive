...
class cluster(object):
    ...
    def __init__(self):
        ...
        self.goalAllocCall()
        # ======================================================================
        # Bool variables initialisation
        # ======================================================================
        self.params_loaded = False
        self.allocation_finished = False
        self.constraints_array = []

    def goalAllocCall(self):
        # ======================================================================
        # System inputs to implement the ditrubution of the goals
        # ======================================================================
        self.goals_file =  argv[1]
        self.robot_file =  argv[2]
        self.ontology_file =  argv[3]
        self.waypoint_file = argv[4]
        self.center_no = int(argv[5])
        self.robots_number = int(argv[5])
        self.total_iteration = 500

        # ======================================================================
        # Task Allocation strategy considering homogeneous robots which is
        # based on k-means algorithm
        # info: we start assuming we know the coordinates of mission goals and
    ...
    def hrta_strategy(self):
        ...
        [self.region_label] = self.iterateForRobots()
        # ======================================================================
        # if the robots are not in the same area (one cluster assocciated with
        # robot coordinates) the distribution of the goals depends just in the
        # Goal Allocation part of the Regions Distribution.
        # Please check paper!!!
        # ======================================================================
        if len(self.region_label) == 1:
            ...
            # ==================================================================
            # Calculates the number of solvable goals for each robot in
            # different clusters
            # ==================================================================
            ...
            # ==================================================================
            for i in range(0, len(self.robot_name)):
            ...
            robot_ind, cluster_ind = linear_sum_assignment(cost)

            for n in range(0, len(self.clus_index)):
                self.index_for_g.append(self.reordering_goals[robot_ind[n]][cluster_ind[n]])
                self.waypoints_initial = []
                for r in range(0, len(self.goals_index)):
                    if self.goals_index[r] in self.reordering_goals[robot_ind[n]][cluster_ind[n]]:
                        self.waypoints_initial.append(self.goal_order[r])
                self.waypoints.append(self.waypoints_initial)
            # ==================================================================
            # Calculates the distance between the robot allocated to a
            # particular cluster and the coordinates of the goals in the cluster.
            # Assigns the first goal (the closest one in the cluster) to this robot
            # ==================================================================
            self.dist_set_initial = []
            goal_set_no = 0
            for robot_coord in range(0, len(self.robot_name)):
                if robot_coord in robot_ind:
                    r_point = np.array(self.robot_data_points[robot_coord])
                    self.initial_point = r_point
                    self.dist_set_initial = []
                    for g_point in range(0, len(self.goals_index)):
                        if str(self.goals_index[g_point]) in self.index_for_g[goal_set_no]:
                    ...
                    if not len(self.dist_set_initial) == 0:
                        ...
                        new_wp = self.waypoints[goal_set_no][self.m_dist_index]
                        ...
                        indx_goal_ordered = self.waypoints[goal_set_no].index(new_wp)
                        self.goal_to_remove = str(self.index_for_g[goal_set_no][indx_goal_ordered])
                    ...
                    goal_set_no = goal_set_no +1

            # ==================================================================
            # Add the goal allocated to the list on allocated goals and removes
            # it from the list of not allocated ones
            # ==================================================================
            goal_counter = 0
            for constraints in range(0,len(self.new_centroids)):
                if not self.robot_points_id[constraints] in self.initial_robot_wp:
                    self.constraints_array.append([self.robot_name[constraints],self.robot_points_id[constraints], self.act_goal[goal_counter]])
                    goal_counter = goal_counter + 1

    def goal_distribution(self):
        # ======================================================================
        # Alloctes the goals in the non allocated goals list considering a cost
        # function that combines goals makespan, distance and redundancy of the
        # sensory system
        # ======================================================================
        self.makespan_accumulated = np.zeros(len(self.robot_points_id))
        self.accumulative_goal = []
            ...
            for goal_set_indx in range(0, len(self.goals_index)):
                ...
                for robot_indx in range(0, len(self.centroids)):
                    if str(self.goals_index[goal_set_indx]) in self.tasks_index[robot_indx]:
                       ...
                       action_makespan = self.makespan_list['makespan_goals'][goal_name]
    ...
    def create_centroids(self):
        ...
        for self.number_robots in range(0, self.center_no):
            self.number_robots = [0.0, 0.0, 0.0]
            self.centroids.append(self.number_robots)
            ...
            for line_no in range(0, lines):
                ...
                for test_line_no in range(0, test_lines):
                    ...
                    if (self.value.find(name+",") != -1):
                        ...
                        curr = next + 1
                        next = reader.find(",",curr)
                        self.z_coord = reader[curr:next]

                        self.data_points.append([float(self.x_coord), float(self.y_coord), float(self.z_coord)])
                ...
                for robot_line_no in range(0, robot_lines):
                    ...
                    if robot_name in reader and not robot_name in self.robot_name:
                        ...
                        curr = next + 1
                        next = reader.find(",",curr)
                        self.z_robot_coord = reader[curr:next]

                        self.robot_data_points.append([float(self.x_robot_coord), float(self.y_robot_coord), float(self.z_robot_coord)])
    ...
    def load_cap_analyser_results(self):
        ...
        for self.i_line in range(0, self.robots_number):
            self.goal_distribution_data = open('goals_indx_distribution.txt', 'rw')
            self.goal_redundancy_data = open('goals_redundancy.txt', 'rw')
    ...
    def load_makespan(self):
        ...
        # ======================================================================
        with open(str(self.ontology_file), 'r') as stream:
    ...
    def assign_label_robot_cluster(self):
        # ======================================================================
        # Assigns a lable to a new cluster
        # ======================================================================
        self.index_of_min = min(self.distance_val, key=self.distance_val.get)
        return [self.index_of_min, self.robot_data_points[self.index_point], self.centroids[self.index_of_min]]
    ...
    def iterateForRobots(self):
        # ======================================================================
        # Obtain an accurate value clusters centroids
        # ======================================================================
        self.label = []
        self.final = []
        self.t_points = len(self.robot_data_points)
        km = len(self.centroids)

        for iteration in range(0, self.total_iteration):
            for self.index_point in range(0, self.t_points):
                self.distance_val = {}
                for self.index_centroid in range(0, km):
                    point = self.robot_data_points[self.index_point]
                    centroid = self.centroids[self.index_centroid]
                    self.initial_point = point
                    self.final_point = centroid
                    self.distance_val[self.index_centroid] = self.compute_euclidean_distance()
                self.label = self.assign_label_robot_cluster()
                self.centroids[self.label[0]] = self.compute_new_centroids()

                if iteration == (self.total_iteration - 1):
                    if not self.index_of_min in  self.final:
                       self.final.append(self.index_of_min)
        return [self.final]
    ...
    def save_goals(self):
        ...
        for i in range(0, len(self.initial_robot_wp)):
            self.allocation_statement = "(robot_can_act" + " " +str(self.robot_name[i])+" "+str(self.initial_robot_wp[i])+")"
            self.allocation_results.append(self.allocation_statement)

        self.allocation_data = open('allocation_solution.txt', 'w+')
